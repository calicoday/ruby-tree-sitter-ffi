...
# src/rusty/gen_rusty.rb, of the tree-sitter test:
...
def test_root_node_with_offset()
    parser = TreeSitterFFI.parser()
    parser.set_language(get_language("javascript"))
    tree = parser.parse("  if (a) b", nil)

    node = tree.root_node_with_offset(6, TreeSitterFFI::Point.new(2, 2))
    assert_eq!(node.byte_range(), 8..16)
    assert_eq!(node.start_point(), TreeSitterFFI::Point.new(2, 4))
    assert_eq!(node.end_point(), TreeSitterFFI::Point.new(2, 12))

    child = node.child(0).child(2)
    assert_eq!(child.type(), "expression_statement")
    assert_eq!(child.byte_range(), 15..16)
    assert_eq!(child.start_point(), TreeSitterFFI::Point.new(2, 11))
    assert_eq!(child.end_point(), TreeSitterFFI::Point.new(2, 12))
    ...
    child = cursor.node()
    assert_eq!(child.type(), "parenthesized_expression")
    assert_eq!(child.byte_range(), 11..14)
    assert_eq!(child.start_point(), TreeSitterFFI::Point.new(2, 7))
    assert_eq!(child.end_point(), TreeSitterFFI::Point.new(2, 10))
end
